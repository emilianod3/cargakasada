        ...
        if ($validator->fails()) {
            ...
            if(ENV('CONTATO_ABRE_OUVIDORIA') == 1){
                try {
                    ...
                    Tools::setAtividade(0, 8, 0, 'Formulário de Contato', 'Criação de Ouvidoria Formulário de Contato Nome:'.$request->contact_name.' e-Mail: '.$request->contact_email);
                    //$result = true;
                } catch (\Exception $e) {
                    $except = $e->getMessage();
                    //return Tools::setResponse('fail', null, 'Não foi possível Processar');
                    Tools::setAtividade(0, 8, 0, 'Formulário de Contato', 'Falha na Criação de Ouvidoria - Formulário de Contato');
                    $result = false;
                }
            }

            if(ENV('ENVIOEMAIL') == 1 && strlen(ENV('EMAIL_FALECONOSCO')) > 4){
                try {
                    $emaildata = new stdClass();
                    $emaildata->subject = 'Formulário de Contato '.ENV('APP_NAME');
                    $emaildata->dados = 'Mensagem Originada de '.ENV('APP_NAME');
                    ...
                    Tools::setAtividade(0, 8, 0, 'Formulário de Contato', 'Envio de E-mail Nome:'.$request->contact_name.' e-Mail: '.$request->contact_email);
                    //return Tools::setResponse('success', null, 'Formulário de Contato');
                } catch (\Exception $e) {
                    $except = $e->getMessage();
                    //return Tools::setResponse('fail', null, 'Não foi possível Processar');
                    Tools::setAtividade(0, 8, 0, 'Formulário de Contato', 'Falha no Envio de E-mail - '.$except);
                    $result = false;
                }        

...
onMounted(() => {
    if (campoNome.value && !form.contact_name) {
    ...
    setTimeout(() => {
        if (window.grecaptcha && typeof window.grecaptcha.render === 'function' && recaptchaContainer.value) {
            widgetId = window.grecaptcha.render(recaptchaContainer.value, {
                ...
                'expired-callback': () => { form.grecaptcha = ''; },
                'error-callback': () => { form.grecaptcha = ''; }
            });
        }
    }, 50);
}
...
const submeterContato = () => {
    ...
    form.post(route('reportarproblema'), {
        onError: (errors) => {   
        ...
        onSuccess: () => {
            ...
            //form.reset('contact_assunto', 'contact_message', 'grecaptcha', 'files');
            /*if (window.grecaptcha && widgetId !== null) {
                window.grecaptcha.reset(widgetId);
            }*/
            form.reset();
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-comum pb-4 gap-2">
                <div>
                    <h1 class="text-2xl font-bold text-texto-claro flex items-center gap-2">
                        <i class="fas fa-envelope text-primary"></i> Reportar um Problema
                    </h1>
                ...
                <form @submit.prevent="submeterContato" class="flex flex-col gap-5">
                    <div class="grid grid-cols-1 gap-4">
                        <div class="flex flex-col gap-1">
                            ...
                            <input v-model="form.contact_name" ref="campoNome" type="text" minlength="5" maxlength="130" class="w-full p-2.5 rounded-lg border border-comum bg-layout-fundo text-texto-claro focus:border-primary focus:ring-1 focus:ring-primary focus:outline-none text-sm transition-all" />
                        </div>
                    </div>
                        ...
                        <div class="flex flex-col gap-1">
                            ...
                            <input
                                ...
                                :class="['w-full p-2.5 rounded-lg border bg-layout-fundo text-texto-claro text-sm transition-all focus:outline-none focus:ring-1',
                                    (form.contact_email && !sistemajs.validacaoEmail(form.contact_email))
                                        ? 'dadoinvalido' 
                                        : 'border-comum focus:border-primary focus:ring-primary']"
                            />
                            <p v-if="form.contact_email && !sistemajs.validacaoEmail(form.contact_email)" class="text-xs dadoinvalido-texto mt-1 font-medium">
                    ...
                    <div class="flex flex-col gap-1">
                        ...
                        <input v-model="form.contact_assunto" type="text" maxlength="150" placeholder="" class="w-full p-2.5 rounded-lg border border-comum bg-layout-fundo text-texto-claro focus:border-primary focus:ring-1 focus:ring-primary focus:outline-none text-sm transition-all" />
                    </div>
